...
- ✅ Story 1.6 — `<PhotoUploader>` (drag-drop + click, 5-state surface `empty|uploading|validating|validated|error`, in-flight concurrency guard, `ImageBitmap.close()` lifecycle) + `<ConsentPrompt>` (non-dismissable Radix Dialog — ESC, outside-click, and close button all blocked; FR46 re-prompt resets choice to `"local"` on every re-open; double-click guard via `submitting` state) + consent state machine in [`src/lib/security/consent-state.ts`](src/lib/security/consent-state.ts) (pure TS, `LEGAL_TRANSITIONS`, `ConsentInvariantError`, `buildConsentRecord`) + Zustand consent store in [`src/lib/stores/consent.ts`](src/lib/stores/consent.ts) (TOCTOU-safe `grant()` — state written before fetch; non-ok HTTP audit failures logged; best-effort POST) + BIPA/CUBI/GDPR copy in [`src/lib/copy/consent.ts`](src/lib/copy/consent.ts) (deep-frozen `consentCopy`) + Zod schema in [`src/lib/schemas/consent.ts`](src/lib/schemas/consent.ts) (UUID-regex `sessionId`) + `/api/consent/grant` stub route handler + `FaceLandmarker` integration in `MockARProvider` (shared `FilesetResolver` — AC9; `DETECT_FAILED` ProviderError; dispose TOCTOU fix) + FR46 Playwright e2e scaffold (`consent-flow.spec.ts`, Chromium-only, `test.fixme` until fixture lands). 279 unit tests / 81 Storybook stories. Bundle **171.24 KB gzipped**. ([dev guide](../_bmad-output/implementation-artifacts/1-6-build-photouploader-consentprompt-with-consent-state-machine.md))
- ✅ Story 1.7 — `<ColorRender>` texture-preserving WebGL2 renderer (GLSL ES 3.00 two-texture pipeline: RGBA photo + R8 mask, Lab color transform, per-pixel mask gate) + `<RenderConfidenceBanner>` (shown when `confidence < 0.55`, UX honesty pattern #2) + `useTryOnStore` Zustand store (`selectedColorId`, `lightingPreset`, `fadeWeek`, `washesPerWeek`, `scrubFade`, `setWashesPerWeek`) + `useTryOnParams` URL-state hook (`?color=`, `?lighting=`, `?week=`) + `color-catalog.ts` (24 OKLCH color swatches) + `render.ts` copy module. Canvas2D HSL fallback for WebGL2-incapable devices; `webglcontextlost` recovery with shader re-compile. 314 unit tests / 34 Storybook stories. Bundle **171.63 KB gzipped**. ([dev guide](../_bmad-output/implementation-artifacts/1-7-build-colorrender-component-texture-preserving-webgl2.md))
- ✅ Story 1.8 — `<FadeSimulator>` 90-day fade timeline: controlled Radix Slider (0–90 days, 3 milestone snap-points at week 2/4/8, visual emphasis within 3-day threshold) + Radix Select (washes/week 1–4), phase-aware week labels (`weekFreshLabel` / `weekModerateLabel` / `weekMaxLabel`), URL-persisted `?week=N` (integer-safe via `Math.round`), Zustand store synced on every drag frame + on mount. Live region (`role="status" aria-live="polite" aria-atomic="true"`) for AT. Reduced-motion CSS-only (`motion-reduce:transition-none`). `useTryOnParams` URL validator capped at ≤13 weeks. 9 adversarial code-review patches applied. 426 unit tests / 91 Storybook stories. Bundle **171.63 KB gzipped**. ([dev guide](../_bmad-output/implementation-artifacts/1-8-build-fadesimulator-90-day-fade-timeline.md))
- 📋 Story 1.9 (next) — `<LightingToggle>` with 3 calibrated presets (indoor 3200K / daylight 5500K / salon 4500K)
- 📋 Stories 1.10 → 8.7 — backlog (see [`sprint-status.yaml`](../_bmad-output/implementation-artifacts/sprint-status.yaml))
