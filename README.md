...
- ✅ Story 1.6 — `<PhotoUploader>` (drag-drop + click, 5-state surface, in-flight guard, `ImageBitmap` lifecycle) + `<ConsentPrompt>` (non-dismissable Radix Dialog, ESC/outside-click blocked, FR46 re-prompt resets to default on re-open) + consent state machine (`src/lib/security/consent-state.ts` — pure TS, `LEGAL_TRANSITIONS`, `ConsentInvariantError`) + Zustand consent store (TOCTOU-safe `grant()`, best-effort audit POST with non-ok HTTP logging) + BIPA/CUBI/GDPR-approved copy module (deep-frozen `consentCopy`) + Zod schema (UUID-validated `sessionId`) + `/api/consent/grant` stub route handler + `FaceLandmarker` integration in `MockARProvider` (shared `FilesetResolver`, `DETECT_FAILED` guard, dispose TOCTOU fix) + FR46 Playwright e2e scaffold. 279 unit tests / 81 Storybook stories. Bundle 171.24 KB gzipped.
- ✅ Story 1.7 — `<ColorRender>` texture-preserving WebGL2 renderer (GLSL ES 3.00 two-texture pipeline: RGBA photo + R8 mask, Lab color transform, per-pixel mask gate) + `<RenderConfidenceBanner>` (shown when `confidence < 0.55`) + `useTryOnStore` Zustand store (`selectedColorId`, `lightingPreset`, `fadeWeek`, `washesPerWeek`, `scrubFade`, `setWashesPerWeek`) + `useTryOnParams` URL-state hook + `color-catalog.ts` (24 OKLCH color swatches) + `render.ts` copy module. Canvas2D HSL fallback for WebGL2-incapable devices; `webglcontextlost` recovery. 314 unit tests / 34 Storybook stories. Bundle 171.63 KB gzipped.
- ✅ Story 1.8 — `<FadeSimulator>` 90-day fade timeline: controlled Radix Slider (0–90 days, 3 milestone snap-points at week 2/4/8) + Radix Select (washes/week 1–4), phase-aware week labels (`weekFreshLabel` / `weekModerateLabel` / `weekMaxLabel`), URL-persisted `?week=N` (integer-safe), Zustand store sync on every drag frame + on mount. Live region (`role="status" aria-live="polite"`) for AT. Reduced-motion CSS-only. 9 adversarial code-review patches applied (url range validator capped at ≤13 weeks, fractional-week URL fix, mount-sync via `initialFadeWeekRef`, runtime wash-value guard, and more). 426 unit tests / 91 Storybook stories. Bundle 171.63 KB gzipped.
